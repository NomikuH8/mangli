...
class Mangli:
    ...
    def main(self):
        ...
        questions = [
            {
                ...
                "choices": ["Homepage", "Search", "Specify a url", "Exit Mangli"],
            }
        ]
        ...
        if answer["entry"] == "Homepage":
            self.homepage(0)
        if answer["entry"] == "Search":
            self.search()
        if answer["entry"] == "Specify a url":
            pass
        if answer["entry"] == "Exit Mangli":
            sys.exit(0)
    ...
    def manga_page(self, manga_id, return_to_func, manga={}):
        ...
        manga_path = rf"{configs.DOWNLOAD_PATH}/mangas/{manga_title}"
        ...
        while len(answer) == 0:
            ...
            cache_path = rf"{configs.DOWNLOAD_PATH}/.cache"
            cover_path = rf"{cache_path}/[cover] {manga_title}.png"
            if len(answer) != 0:
                if answer["manpage"] == "Show cover":
                    ...
                    if not os.path.exists(cover_path):
                        ...
                        utils.start_image_app(cover_path)
                    self.manga_page(manga["id"], return_to_func, manga)
                if answer["manpage"] == "Read":
                    self.vol_read_menu(manga, return_to_func)
    ...
    def vol_read_menu(self, manga, page_return):
        ...
        choices.append(
            Separator("--- And please put your image viewer to sort by name")
        )
    ...
    def chap_read_menu(self, sel_vol, manga, page_return):
        ...
        if answer["chapter"] == "Return":
            self.vol_read_menu(manga, page_return)
        ...
        for i in sel_vol.chapters.keys():
            if answer["chapter"] == i:
                ...
                first_page_path = (
                    rf"{cache_path}{validated_title}/vol_{volume_num}/chap_{i}/1.png"
                )
